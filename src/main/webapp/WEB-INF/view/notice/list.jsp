<%@page import="com.leesh.web.entity.Notice"%>
...
<%-- 			   <%
 ...
 --%>
 					<c:forEach var="notice" items="${list}" >
 						<tr>
							<td>${notice.id}</td>
							<td class="title indent text-align-left"><a href="detail?id=${notice.id}">${notice.title}</a><span>[${notice.commentCnt}]</span></td>
...
<%-- 					<%} %>							
 --%>							
		    ...
		    <c:set var="lastNum" value="${fn:substringBefore(Math.ceil(totalCnt/10), '.')}" />
			...
			<div class="margin-top align-center pager">	
				...
				<div>
					<c:if test="${(startNum+10) <= lastNum }">
					 	<a href="?p=${startNum+10}&f=${param.f}&q=${param.q}" class="btn btn-next">다음</a>
					</c:if>
					<c:if test="${(startNum+10) >= lastNum }">
